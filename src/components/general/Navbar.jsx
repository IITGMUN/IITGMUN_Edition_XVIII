...
import mun from "./Group 63.png";
import alcher from "./Mask group.png";
      ...
      <div className="pointer-events-auto flex-shrink-0">
        <NavLink to="/">
          <img
            src={mun}
            alt="IITG MUN Logo"
            // Increased size: h-16 (mobile) to md:h-20 (desktop)
            className="h-14 w-auto md:h-16 object-contain drop-shadow-sm hover:scale-105 transition-transform"
          />
        </NavLink>
      </div>

      {/* 2. CENTER NAVIGATION (Squarish Round) */}
      {/* Container: rounded-2xl for squarish look */}
        ...
        {navLinks.map((link) => (
          <NavLink
            ...
            className={({ isActive }) => `
              ...
              rounded-xl  // <--- Makes items squarish-round
              ${
                isActive
      ...
      <div className="pointer-events-auto flex-shrink-0">
        <a href="https://alcheringa.in/home" target="_blank" rel="noreferrer">
          <img
            src={alcher}
            alt="Alcheringa Logo"
            // Increased size: h-12 (mobile) to md:h-16 (desktop)
